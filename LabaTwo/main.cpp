...

{
	...
	glViewport(0, 0, width, height);//это область окна, в которой будет отображаться результаты нашей работы|x и y - координаты левого верхнего угла области видимости, width и height - размеры

	glMatrixMode(GL_PROJECTION);//установки текущей матрицы|GL_PROJECTION - все последующие изменения будут применяться к проекционной матрице

	glLoadIdentity();//текущая матрица будет сделана единичной

	glOrtho(-20, 20, -20, 20, 1, 17);//Указывает размеры плоскости

	gluLookAt(0, 0, 8, 0, 0, 0, 0, 2, 0);//Первые 3 - координаты позиции глаза налюдателя, вторые 3 - координаты точки, распологающейся в центре экрана, третьи 3 - направление вектора, задающего поворот сцены
